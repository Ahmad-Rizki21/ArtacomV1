                            ...
                            Select::make('pelanggan_id')
                                ...
                                ->afterStateUpdated(function ($state, callable $set, $get) {
                                    if ($state) {
                                        ...
                                        if ($pelanggan) {
                                            $langganan = Langganan::where('pelanggan_id', $pelanggan->id)->first();
                                            ...
                                            if ($langganan) {
                                                ...
                                                Log::info('Data langganan ditemukan', [
                                                ...
                                                $set('info_layanan', 'Tidak ada data langganan');
                                                $set('info_brand', 'Tidak ada data langganan');
                                                $set('info_status', 'Tidak ada data langganan');
                                                
                                                Log::warning('Pelanggan tidak memiliki langganan', [
                                                    'pelanggan_id' => $pelanggan->id
                                                ]);
                                            }
                                        }
                            ...
                            Grid::make(3)
                                ->schema([
                                    TextInput::make('info_layanan')
                                        ...
                                        ->disabled()
                                        ->dehydrated(false)
                                        ->default(fn ($record) => $record ? ($record->pelanggan->langganan->layanan ?? 'Tidak ada data') : 'Tidak ada data'),
                                    ...
                                    TextInput::make('info_brand')
                                        ...
                                        ->disabled()
                                        ->dehydrated(false)
                                        ->default(fn ($record) => $record ? ($record->pelanggan->langganan->id_brand ?? 'Tidak ada data') : 'Tidak ada data'),
                                    ...
                                    TextInput::make('info_status')
                                        ...
                                        ->disabled()
                                        ->dehydrated(false)
                                        ->default(fn ($record) => $record ? ($record->pelanggan->langganan->user_status ?? 'Tidak ada data') : 'Tidak ada data'),
                                ]),
                        ...
                        ->schema([
                            Card::make()
                                ...
                                ->schema([
                                    ...
                                    // Tambahkan File Upload untuk Bukti Speedtest
                                    \Filament\Forms\Components\FileUpload::make('speedtest_proof')
                                        ->label('Bukti Speedtest')
                                        ->disk('public') // Disk tempat file disimpan
                                        ->directory('speedtest_proofs') // Direktori penyimpanan
                                        ->acceptedFileTypes(['image/*', 'application/pdf']) // Hanya gambar dan PDF
                                        ->maxSize(2048) // Maksimal 2MB
                                        ->helperText('Upload bukti speedtest (opsional, maksimal 2MB, format: gambar atau PDF)')
                                        ->required(false) // Tidak wajib
                                        ->imagePreviewHeight('200') // Preview untuk gambar
                                        ->columnSpan('full'),
                                ]),
                        ]),
                ])
    ...
    public static function table(Tables\Table $table): Tables\Table
{
    return $table
        ->columns([
            ...
            // Kolom baru untuk Bukti Speedtest
